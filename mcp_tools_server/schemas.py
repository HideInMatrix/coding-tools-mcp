...
TOOL_SPECS: dict[str, ToolSpec] = {
    ...
    "discover_toolchains": ToolSpec("discover_toolchains", "Discover toolchains", "Discover validated Node.js, Python, and Go toolchains in the sandbox first; if absent, request permission before querying the user environment.", obj({"kinds": {"type": "array", "items": {**S, "enum": ["node", "python", "go"]}, "default": ["node", "python", "go"]}}), True, False, True),
    ...
    "exec_process": ToolSpec("exec_process", "Execute process", "Run a structured process without accepting a user-provided shell command string.", obj({"program": {**S, "minLength": 1}, "args": {"type": "array", "items": S, "default": []}, "workdir": {**S, "default": "."}, "cwd": S, "timeout_ms": {**I, "minimum": 1, "maximum": 600_000, "default": 30_000}, "yield_time_ms": {**I, "minimum": 0, "maximum": 30_000, "default": 10_000}, "stdin": {**S, "default": ""}, "tty": {**B, "default": False}, "env": {"type": "object", "additionalProperties": {"type": "string"}, "default": {}}, **EXEC_COMMON}, ("program",)), False, True, False, True),
    ...
    "request_permissions": ToolSpec("request_permissions", "Request permissions", "Request an explicit client-side permission confirmation when the client supports MCP elicitation; never silently escalates privileges.", obj({"tool_name": {**S, "enum": ["discover_toolchains", "exec_process", "exec_command", "apply_patch"]}, "permission": {**S, "enum": ["network", "destructive_command", "git_metadata_write", "long_timeout", "sensitive_env", "sandbox_env_override", "shell_expansion", "inline_script", "privileged_executable", "write_generated_or_ignored"]}, "reason": {**S, "minLength": 1}, "arguments": {"type": "object", "additionalProperties": True}, "scope": {**S, "enum": ["once", "session"], "default": "once"}, "ttl_seconds": {**I, "minimum": 1, "maximum": 3_600, "default": 300}}, ("tool_name", "permission", "reason", "arguments")), True),
