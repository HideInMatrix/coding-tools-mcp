    ...
    for attempt in range(1, UPDATE_DOWNLOAD_ATTEMPTS + 1):
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "Coding-Tools-MCP-Updater",
                "Connection": "close",
            },
        )
        try:
...
def _write_helper_script(suffix: str, content: str) -> Path:
    fd, raw_path = tempfile.mkstemp(
        prefix="coding-tools-mcp-updater-",
        suffix=suffix,
        text=True,
    )
    path = Path(raw_path)
...
STAGING="$(mktemp -d -t coding-tools-mcp-update.XXXXXX)"
...
def _spawn_update_helper(archive: Path, target: Path) -> None:
    ...
    if system == "darwin":
        ...
        script = _write_helper_script(".ps1", _WINDOWS_HELPER)
        powershell = shutil.which("powershell.exe") or shutil.which("powershell")
        if not powershell:
            script.unlink(missing_ok=True)
            raise RuntimeError("系统未找到 PowerShell，无法执行自动更新。")
        command = [
            powershell,
            "-NoProfile",
            "-NonInteractive",
...
class UpdateManager:
    ...
    def _download_worker(self) -> None:
        ...
        try:
            ...
            temp_dir = Path(tempfile.mkdtemp(prefix="coding-tools-mcp-update-download-"))
